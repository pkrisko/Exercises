 // ## Array Cardio Day 2
 const currYear = new Date().getFullYear();

 const people = [
   { name: 'Wes', year: 1988 },
   { name: 'Kait', year: 1986 },
   { name: 'Irv', year: 1970 },
   { name: 'Lux', year: 2015 }
 ];

 const comments = [
   { text: 'Love this!', id: 523423 },
   { text: 'Super good', id: 823423 },
   { text: 'You are the best', id: 2039842 },
   { text: 'Ramen is my fav food ever', id: 123523 },
   { text: 'Nice Nice Nice!', id: 542328 }
 ];

 // Some and Every Checks

 // Array.prototype.some() //
 console.log("is at least one person 19 or older?");
const hasAdult = people.some((person) => currYear - person.year >= 19);
console.log(hasAdult);

 // Array.prototype.every() //
 console.log("is everyone 19 or older?");
const allAdults = people.every((person) => currYear - person.year >= 19);
console.log(allAdults);

 // Array.prototype.find()
 // Find is like filter, but instead returns just the one you are looking for
console.log("find the comment with the ID of 823423");
const comment = comments.find((comment => comment.id === 823423));
console.log(comment);

 // Array.prototype.findIndex()
 // Find the comment with this ID
 console.log("delete the comment with the ID of 823423");
const index = comments.findIndex((comment) => comment.id === 823423);
console.log(index);

console.log("Removing the comment with this index ^");
console.log("Old Comments: ", comments);
const newComments = [
...comments.slice(0, index),
...comments.slice(index + 1)
];
console.log("New Comments: ", newComments)
